...
int add_mass_monomer()
{
  ...
  if( (mass_monomer[f][x1] == 0)     && (mass_monomer[f][x2] == 0) &&
      (occupation_field[f][x1] == 0) && (occupation_field[f][x2] == 0)){
...
int remove_mass_monomer()
{
  ...
  if( (mass_monomer[f][x1] == 1)     && (mass_monomer[f][x2] == 1) &&
      (occupation_field[f][x1] == 1) && (occupation_field[f][x2] == 1)){
...
void update_fourfermion( int *additions, int *removals, int *moves )
{
  int changes=0;
  update_block();

  for( int i=0; i<N_BLOCK_UPDATES; i++){
    if(mersenne()>0.5){
...
void update_mass( int *additions, int *removals, int *moves )
{
  int changes=0;
  update_block();
  ...
  for( int i=0; i<N_BLOCK_UPDATES; i++){
    if(mersenne()>0.5){
      *removals += remove_mass_monomer();
    } else {
      *additions += add_mass_monomer();
    }
    //*moves += move_mass_monomer();
    switch_monomers();
  }
}
...
int main(int argc, char* argv[])
{
  ...
  int additions = 0, removals = 0, moves = 0;
  ...
  for (i=1; i<n_measure+1; i++) {
    ...
    /* updates */
    /* Do a number of updates to the four fermion monomers */
    for(int k=0; k<n_updates_per_measure; k++){
        update_fourfermion( &additions, &removals, &moves );
        update_mass(&m_additions, &m_removals, &m_moves);
    }
    ...
    printf("\n%d, %d updates in %.3g seconds, %d additions, %d removals, %d moves \n", i, n_updates_per_measure, 1e-6*diff, additions, removals, moves );
    ...
    //for(int j=0; j<n_average; j++){
      /* Then the mass monomers (with small mass, this is fast) */
    //  for(int k=0; k<n_mass_updates_per_measure; k++) update_mass( &m_additions, &m_removals, &m_moves );
    ...
    printf("%d, %d mass updates in %.3g seconds, %d additions, %d removals, %d moves \n", i, n_mass_updates_per_measure*n_average, 1e-6*diff, m_additions, m_removals, m_moves );
    m_additions = 0; m_removals = 0; m_moves = 0;

    printf("MONOMERDENSITY %g ", fourfermion_monomer_density);
